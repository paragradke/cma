...
.factory('CustomerService',  ['$http', function($http) {
        ...
        return {
            ...
            updateStatus: function (customerId, propertyName, newStatus, success, error) {
                console.log("Customer Service addProperty");
                $http.post(baseUrl + '/api/updatepropertystatus/customers/'+ customerId+'?propertyName='+propertyName+'&status='+newStatus)
                    .success(success)
                    .error(error);
            },
